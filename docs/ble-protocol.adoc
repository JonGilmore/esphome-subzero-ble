...
NOTE: We've previously misdiagnosed "fw 8.5 unlock session expiry" as the root cause of silent poll failures on wall ovens and 313 fridges. The actual cause was polling D5 (the control channel) instead of D6 (the data channel). D6 returns full state on demand on both firmware versions with no session expiry behavior. The push-only / subscribe-retry workarounds we previously needed are gone.
