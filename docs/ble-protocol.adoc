...
. **parse_json scripts:** trim the buffer to exact JSON extent (first `{` through matching `}`), then *parse first, publish second*. If the parser rejects the message (malformed, `status!=0`, or a nested object like `{"notif_seq":N,...}` left over from a truncated response), no sensors are updated for that message - pushing partial state from a corrupted poll would be misleading.
