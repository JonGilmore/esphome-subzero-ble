...
+
NOTE: This project previously shipped a `patch_acl_reassembly` external component that overwrote `packet_fragmenter.c` at build time. It was removed once the fix landed upstream; the minimal example configs now declare `esphome: min_version: 2026.7.1` instead. The buffer-handling described below is unchanged - it is defensive and useful regardless.
+
