...
+
The integration mitigates the corruption in two layers:
+
...
. **parse_json scripts:** before publishing the reassembly buffer to the `Raw JSON` text sensor or feeding it to the parser, trim to exact JSON extent (first `{` through matching `}`). Any trailing bytes after the JSON are either preserved (if they start with `{` - likely the beginning of the next message) or discarded (non-JSON garbage). This is required because ESPHome text sensors serialize as protobuf `string`, which must be valid UTF-8 - raw binary crashes Home Assistant's protobuf decoder and, because HA caches the bad state, causes a restart loop.
+
Even with both layers, the *first push notification in a burst* is typically lost at the appliance side - for example, `cav_gourmet_mode_on` during gourmet mode activation. That fragment truly never arrives; no amount of buffer handling can recover it. Subsequent state changes in the same session are received correctly.
