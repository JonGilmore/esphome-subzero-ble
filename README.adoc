...
====
**If you are upgrading from v0.20.x or earlier**, you must now declare `external_components` in your entry-point YAML file. Previous versions bundled this declaration in `subzero_base.yaml`, but ESPHome package resolution requires it at the top level.

Add this block to your device config (above or below the `packages:` block):
...
Keep the `ref:` matching the one in your `packages:` block so components and YAML come from the same release.
====
...
| The BLE unlock session has expired. The integration detects this: if two consecutive poll commands go unanswered (while pushes may still arrive), it forces a reconnect. Typical recovery is under ~2 minutes from the moment polls stop getting responses. Look for `polls unanswered (unlock session likely expired)` in the logs, or monitor the `no_resp` counter in the periodic poll log line. See link:docs/advanced.adoc#reconnect-logic[Reconnect Logic] for details and tuning.
...
| *Home Assistant keeps restarting with `DecodeError: Error parsing message with type 'TextSensorStateResponse'`*
| This is the ACL corruption (above) leaking non-UTF-8 bytes into a text sensor state, which HA caches and then fails to decode on every reconnect. If you see this on an older firmware, **reboot the ESP** (unplug/replug or `esphome run <config> --device <ip>`) to clear the cached bad state, then flash the latest version - the parse_json scripts now trim to exact JSON extent before publishing, and the notify handlers discard corrupt partial buffers when a new `{` arrives. See link:docs/ble-protocol.adoc#_key_protocol_details[ACL Fragment Drops] for details.
|===
