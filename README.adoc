...
NOTE: **The following fridge features are only available on this fork** (not yet merged upstream): opt-in setpoint writes, the Ice Maker Mode / Appliance Mode / Night Mode / Humidity Control selects, and the Automatic Crisper Temperature / Air Filter switches. See <<fridge-write-support>> below. To use them, point `external_components` at `https://github.com/bob-laz/esphome-subzero-ble` (`ref: main`, or pin to a specific commit) instead of the upstream URL shown in the examples above.
