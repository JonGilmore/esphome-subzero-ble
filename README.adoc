...
| `hide_vacation_ice_modes` | `true` | Long/Short Vacation Mode, High Usage Mode (+ start/end time), Max Ice Mode (+ start/end time), Night Ice Mode, plus the derived Ice Maker Mode / Appliance Mode / Night Mode selects when `enable_mode_selects: true`
| `hide_extra_diagnostics` | `true` | Power On, Pairing Window Open, Door Ajar Alarm Timeout, WiFi Network/Signal/Channel/Encryption Type, Active Faults, plus the derived Humidity Control select and Smart Grid Mode switch when `enable_mode_selects: true`
...
| *Power On* | Whether the unit is powered _(optional, `hide_extra_diagnostics`)_
...
NOTE: **The following fridge features are only available on this fork** (not yet merged upstream): opt-in setpoint writes, the Ice Maker Mode / Appliance Mode / Night Mode / Humidity Control selects, and the Smart Grid Mode / Automatic Crisper Temperature switches. See <<fridge-write-support>> below. To use them, point `external_components` at `https://github.com/bob-laz/esphome-subzero-ble` (`ref: main`, or pin to a specific commit) instead of the upstream URL shown in the examples above.
...
NOTE: **Dishwasher light is intentionally read-only for now.** The appliance acks `set light_on` (status:0) but the actual state never changes. Until that's understood, it stays a read-only binary sensor so a misclick can't risk altering a running appliance. **Fridge set temperatures are a different story** - see <<fridge-write-support>> below, since this fork found the actual reason they looked inert.

NOTE: Writes go to the **D5** control channel (the same one used for unlocking and pairing). The appliance must be connected and paired - if the connection is down or the pairing window is closed (status 302), writes are dropped (now logged as a `WARN` in this fork - see below) and the entity reverts on the next poll. The standard pairing flow is unchanged.

[[fridge-write-support]]
=== Fridge Write Support

WARNING: Everything in this section is **specific to this fork** (`https://github.com/bob-laz/esphome-subzero-ble`) and has not been merged upstream. Point `external_components` at this fork's URL (`ref: main`, or pin to a commit) to use it. Confirmed via live BLE testing against one appliance (Sub-Zero CL4850UFDID, firmware 2.27) on 2026-07-25 - option encodings and write behavior may differ on other models/firmware, so everything here is opt-in and defaults to the safe read-only behavior.
...
<2> Adds the Ice Maker Mode, Appliance Mode, Night Mode, and Humidity Control selects, plus a Smart Grid Mode switch.
...
| *Humidity Control* (`Normal`/`Enhanced`) | Single-field select, confirmed both directions. **Wire encoding is `1`=Normal / `2`=Enhanced** - notably not `0`/`1` like Night Mode. An earlier version of this fork assumed `0`/`1` here, which meant the write silently no-opped (the appliance ignores the invalid value `0`) and the displayed label was backwards.
| *Smart Grid Mode* | Writable switch, but **the write itself is unconfirmed** - it's only ever been observed at its default `true` value and has never been independently toggled and verified.
|===
...
| Sub-Zero | CL4850UFDID | Refrigerator/freezer with ice maker (fw 2.27) | Fully working, including <<fridge-write-support,fork-only write support>> - temp control, Ice Maker Mode, Appliance Mode, Night Mode, Humidity Control, Automatic Crisper Temperature
