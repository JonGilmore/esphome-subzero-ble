...
After upgrading, Home Assistant will see the entity's unit and device class change. If it complains about incompatible units or long-term statistics, delete the old `sensor.<name>_appliance_uptime` entity and let it be recreated. Anything referencing the old string - a template, dashboard card, or automation - needs updating to handle a number of seconds.
====
...
NOTE: Home Assistant's logbook cannot be filtered with `entity_category`. It only auto-excludes numeric sensors carrying a `unit_of_measurement` or `state_class`, so a *text* sensor produces a logbook row for every distinct value it publishes. This is why *Appliance Uptime* is a numeric duration in seconds rather than the raw `H:MM:SS` string the appliance sends - it advances on every poll, and as a text sensor it wrote a logbook row every poll.
