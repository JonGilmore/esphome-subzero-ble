...
NOTE: Home Assistant's logbook cannot be filtered with `entity_category`; it only auto-excludes numeric sensors that carry a `unit_of_measurement` or `state_class`. A text sensor therefore produces a logbook row for every distinct value it publishes. The *Uptime* sensor reports the appliance's own uptime clock, so it changes on every successful poll and is inherently chatty.

To hide any of these from the logbook while keeping full history and state, filter them in Home Assistant (this affects the logbook only - `recorder:` history is untouched):
